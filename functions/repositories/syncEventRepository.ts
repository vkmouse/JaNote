...
export async function getPullEventsForSharedUsers(
  ...
  let query = `SELECT id, mutation_id, entity_type, entity_id, payload FROM sync_events WHERE user_id IN (${userPlaceholders}) AND id > ? AND entity_type IN ('CAT', 'TXN', 'BGT', 'RTXN', 'RBGT', 'AST')`;
