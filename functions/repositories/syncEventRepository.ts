...
export async function getMaxSyncEventId(userId: string, DB: D1Database): Promise<number> {
  const row = await DB.prepare('SELECT MAX(id) as max_id FROM sync_events WHERE user_id = ?')
    .bind(userId)
...
export async function getPullEvents(
  ...
  let query = 'SELECT id, mutation_id, entity_type, entity_id, payload FROM sync_events WHERE user_id = ? AND id > ?';
...
export async function getMaxSyncEventIdForUsers(
  ...
  const row = await DB.prepare(`SELECT MAX(id) as max_id FROM sync_events WHERE user_id IN (${placeholders})`)
    .bind(...userIds)
    .first<{ max_id: number | null }>();
  return row?.max_id ?? 0;
}
